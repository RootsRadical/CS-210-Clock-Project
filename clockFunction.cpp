...
int AddTime(int standardHour, int militaryHour, int sharedMinutes, int sharedSeconds, int selection) {
	...
	else if (selection == 2) {				/* function used when a user is adding time */
		sharedMinutes += 1;
	}
	else if (selection == 3) {
		sharedSeconds += 1;
...
int UpdateTime(int standardHour, int militaryHour, int minutes, int seconds) {
	...
	if (minutes > 59) {
		standardHour += 1;
		militaryHour += 1;			/* function that updates the time. */
		minutes = 0;
	}
	if (standardHour > 12) {
		standardHour = 1;
...
int DisplayTime(int standardHour, int militaryHour, int minutes, int seconds) {		/* Function displays the time. */
	string meridiem;
